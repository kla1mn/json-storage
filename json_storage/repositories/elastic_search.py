...
class ElasticSearchDBRepository:
    ...
    async def _get_real_index(self, namespace: str) -> str:
        real_index = await self.redis_repository.get_from_dict(
            self.MAPPING_NAMESPACE_INDEX_DICT_NAME, namespace
        )
        if not real_index:
    ...
    async def reindex_namespace(
        self, index: str, real_namespace: str, mappings: MappingsType
    ) -> None:
        async with self.get_client() as client:
            ...
            try:
                ...
                await self.redis_repository.add_to_dict(
                    self.REINDEX_NAMESPACES_DICT_NAME, real_namespace, new_index
                )
                ...
                await self.redis_repository.remove_from_dict(
                    self.REINDEX_NAMESPACES_DICT_NAME, real_namespace
                )
                await client.close()
    ...
    async def create_or_update_index(
        ...
        if not (
            ...
            await self.redis_repository.add_to_dict(
                self.MAPPING_NAMESPACE_INDEX_DICT_NAME, namespace, namespace
            )
        ...
        async with self.get_client() as client:
            ...
            if not exists:
                ...
                return
            if await self.redis_repository.get_from_dict(
                self.REINDEX_NAMESPACES_DICT_NAME, namespace
            ):
